...
export default function ParticipantResult() {
  ...
  const { data, isLoading, error } = trpc.participants.viewResult.useQuery(
    { token: token || "" },
    { enabled: !!token }
  );
  ...
  const { participant, group, drawn, message, allParticipants } = data;
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <Card className="border-2 border-primary/20">
            <CardHeader className="text-center">
              <CardTitle className="text-3xl">Olá, {participant.name}! 🎄</CardTitle>
